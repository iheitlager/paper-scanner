...
class TestStepNavigationAPI:
    ...
    def test_execute_next_step_error_when_complete(self, executor, sample_definition_file):
        """execute_next_step returns final status when no steps remain"""
        executor.load_definition(sample_definition_file)
        executor.current_step_index = len(executor.steps)

        result = executor.execute_next_step()

        assert result["status"] == StepStatus.FINAL
        assert "No more steps" in result["message"]
...
class TestDefinitionLoading:
    ...
    def test_load_definition_without_templates(self, executor, temp_cache_dir):
        ...
        definition = {"project": {"name": "Simple"}, "steps": [{"step": "Echo", "builtin.echo": {"message": "test"}}]}
    ...
    def test_validate_template_references_undefined(self, executor, temp_cache_dir):
        ...
        definition = {
            "project": {"name": "Test"},
            "templates": [{"template": "screening", "steps": [{"step": "Echo", "builtin.echo": {}}]}],
            "steps": [{"step": "Apply template", "builtin.run-template": {"template": "nonexistent"}}],
        }
...
class TestStepExecution:
    ...
    def test_execute_step_out_of_range(self, executor, sample_definition_file):
        """Test executing step index out of range raises StepError"""
        executor.load_definition(sample_definition_file)

        with pytest.raises(StepError, match="out of range"):
            executor.execute_step(999)
    ...
    def test_execute_step_timing(self, executor, sample_definition_file):
        """Test that step timing is recorded in step_history"""
        executor.load_definition(sample_definition_file)

        executor.execute_step(0)

        assert len(executor.step_history) > 0
        history = executor.step_history[0]
        assert "step" in history
        assert "duration_ms" in history
        assert isinstance(history["duration_ms"], int)
    ...
    def test_execute_step_with_override_config(self, executor, sample_definition_file):
        ...
        override_config = {"step": "Override", "builtin.echo": {"message": "overridden"}}
    ...
    def test_execute_step_error_handling(self, executor, sample_definition_file):
        ...
        # Mock step to raise exception - exception should propagate
...
class TestStatistics:
    ...
    def test_get_stats_timing(self, executor, sample_definition_file):
        """Test stats include step history with timing information"""
        executor.load_definition(sample_definition_file)
        executor.run_all()

        stats = executor.get_stats()

        assert "step_history" in stats
        assert len(stats["step_history"]) == 3
        # Each step history should have duration_ms as int
        for history in stats["step_history"]:
            assert "duration_ms" in history
            assert isinstance(history["duration_ms"], int)
...
class TestHaltException:
    ...
    def test_halt_different_from_error(self, executor, sample_definition_file):
        ...
        # Then test error - exception should propagate, not be caught
...
class TestRunAllCallbacks:
    """Tests for run_all functionality"""

    def test_run_all_executes_all_steps(self, executor, sample_definition_file):
        """Test that run_all executes all remaining steps"""
        executor.load_definition(sample_definition_file)
        ...
        assert result.stats["steps_executed"] == 3
        assert result.status == StepStatus.SUCCESS

    def test_run_all_stops_on_error(self, executor, sample_definition_file):
        """Test that run_all stops when a step returns error"""
        executor.load_definition(sample_definition_file)
