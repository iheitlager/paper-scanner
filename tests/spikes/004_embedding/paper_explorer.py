...
class PaperExplorer:
        ...
        while True:
            ...
            try:
                user_input = Prompt.ask(
                    "[cyan]Search or select (1-{})".format(len(display_list)),
                    default=""
                ).strip()
                ...
                try:
                    ...
                    if 0 <= idx < len(display_list):
                        selected_citekey = display_list[idx][0]
    ...
    def get_paper_by_citekey(self, citekey: str) -> dict:
        ...
        paper = {
            ...
            'doi': result[7],
            'keywords': result[8],
            'embedding': np.array(result[9]) if result[9] else None
        }
    ...
    def display_paper(self, paper: dict):
        ...
        console.print()
        console.print(Panel(
            f"[bold cyan]{paper['citekey']}[/bold cyan]",
            border_style="cyan"
        ))
        ...
        if 'cluster' in paper:
            stats_table.add_row(
                "Cluster", 
                f"{paper['cluster']} ({paper.get('cluster_confidence', 0):.1%} confidence)"
            )
...
Focus on insights and connections between papers."""
    ...
    def cmd_search(self):
        ...
        results = self.search_papers(query, limit=10)
        ...
        for i, (citekey, title, year, journal, similarity) in enumerate(results, 1):
            table.add_row(
                str(i),
                citekey,
    ...
    def cmd_similar(self):
        ...
        table = Table(
            title=f"Papers Similar to {self.current_paper['citekey']}", 
            box=box.ROUNDED
        )
        table.add_column("Citekey", style="cyan")
        table.add_column("Title", style="bold")
    ...
    def cmd_compare(self):
        ...
        question = "Compare these two papers. What are the similarities and differences? How do they relate to each other?"
    ...
    def cmd_overview(self):
        ...
        console.print(Panel(
            Markdown(response),
            title="[bold cyan]Collection Overview[/bold cyan]",
            border_style="cyan"
        ))
    ...
    def cmd_clusters(self):
        ...
        for i, (cid, name, count, avg_year) in enumerate(clusters, 1):
            table.add_row(
                ...
                name,
                str(count),
                f"{avg_year:.0f}" if avg_year else "N/A"
            )
            ...
            try:
                ...
                if 0 <= idx < len(clusters):
                    ...
                    if Confirm.ask("[cyan]Ask Claude about this cluster?[/cyan]", default=False):
                        paper_list = "\n".join([
                            f"- {ck} ({year}): {title}"
                            for ck, title, year in papers_in_cluster
                        ])
    ...
    def cmd_network(self):
        ...
        if Confirm.ask("[cyan]Ask Claude for network insights?[/cyan]", default=False):
            ...
            question = "Analyze this citation network. What does it tell us about the collection? Which papers are central?"
            ...
            console.print(Panel(
                Markdown(response),
                title="[bold cyan]Citation Network Insights[/bold cyan]",
                border_style="cyan"
            ))
    ...
    def cmd_list(self):
        ...
        # Get filter key
        filter_key = Prompt.ask(
            "[cyan]Filter by keyword (author, title, year, journal) or leave blank for all[/cyan]",
            default=""
        ).strip().lower()
