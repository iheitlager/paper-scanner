...
class ClusterVisualizer:
    ...
    def plot_cluster_scatter(self, reduced_embeddings, output_path='cluster_scatter.png'):
        ...
        for cluster_id, color in zip(unique_clusters, colors):
            ...
            for i, (x, y, citekey) in enumerate(zip(
                reduced_embeddings[mask, 0],
                reduced_embeddings[mask, 1],
    ...
    def plot_cluster_stats(self, output_path='cluster_stats.png'):
        ...
        for cluster_id in unique_clusters:
            cluster_confidences = [
                float(conf) if conf else 0.0 for cid, conf in zip(self.cluster_ids, self.confidences)
                if cid == cluster_id
            ]
            if cluster_confidences:
                confidence_data.append(cluster_confidences)
                cluster_name = next(
                    (c[1] for c in self.cluster_stats if c[0] == cluster_id),
                    f'Cluster {cluster_id}'
                )
        ...
        if year_cluster_counts:
            ...
            for cluster_id in unique_clusters:
                cluster_data[cluster_id] = [
                    year_cluster_counts[year].get(cluster_id, 0)
