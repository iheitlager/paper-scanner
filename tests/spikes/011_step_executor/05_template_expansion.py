...
def main():
    ...
    if executor.templates:
        ...
        print("   - No templates defined in this definition")
    ...
    for i, step in enumerate(executor.steps):
        ...
        if 'builtin.run-template' in step:
            ...
            print("     → This step uses builtin.run-template")
    ...
    print("   - All templates are defined: ✓")
    ...
    print("\n6. Checkpoint Implications:")
    print("   - Checkpoints are saved by main step index")
    print("   - Template expansion is internal to step execution")
    print("   - Resuming from checkpoint doesn't depend on template content")
    print(f"   - If you resume at step {template_steps_indices[0] if template_steps_indices else 0},")
    print("     the template will be expanded again")
