    ...
    try:
        ...
        for attempt in range(max_poll_attempts):
            ...
            if response.processing_status == "ended":
                print(
                    f"Batch processing ended after {attempt + 1} polling attempts!",
                    file=sys.stderr,
                )
                break
                ...
                if hasattr(response, "status_tallies"):
                    ...
                    print(
                        f"Attempt {attempt + 1}/{max_poll_attempts}: Batch still in progress...",
                        file=sys.stderr,
                    )
            ...
            print(
                f"Exceeded maximum polling attempts ({max_poll_attempts})",
                file=sys.stderr,
            )
            return None
