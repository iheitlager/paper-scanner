"""
RIS import step for paper scanner

Sequentially imports RIS files and adds papers to the database
"""
...
class RisImportStep(BaseStep):
    """RIS import step for adding papers from RIS files."""

    @staticmethod
    def validate(config: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """
        Validate ris_import step configuration.
        ...
        for key in config.keys():
            if key == "limit":
                ...
                if source_database.lower() not in VALID_SOURCE_DATABASES:
                    errors.append(
                        f"'source_database' must be one of {VALID_SOURCE_DATABASES}, "
                        f"got '{source_database}'"
                    )
    ...
    def execute(
        ...
        Raises:
            ConfigurationError: If RIS file cannot be loaded or parsed
        """
        randomize = config.get("randomize", False)
        random_seed = config.get("random_seed", None)
        limit = config.get("limit", None)
        file_path = config.get("file_path")
        source_database = config.get("source_database", "other")
        ...
        if limit:
            if randomize:
                ...
                details.append(
                    f"Randomized papers (seed={random_seed})"
                    if random_seed is not None
                    else "Randomized papers"
                )
            papers = papers[:limit]
        ...
        if expected_count:
            if count != expected_count:
                self.callback(
                    f"[yellow]Warning: Expected {expected_count} papers, "
                    f"got {count}[/yellow]",
                    debug=True
                )
