...
class Stage0Preprocessor:
    ...
    def _check_paper(
        ...
        if paper_id in duplicates:
            # This is a duplicate - will be marked with kept_paper_id pointing to original
            return False, "duplicate"

        for primary_id, dup_ids in duplicates.items():
            if paper_id in dup_ids:
                # This paper is a duplicate of primary_id
                return False, f"duplicate_of_{primary_id}"
    ...
    def process(self) -> None:
        ...
        try:
            ...
            for idx, paper in enumerate(papers):
                ...
                paper_id = paper['id']
                should_include, exclusion_reason = self._check_paper(
                    paper_id,
                    paper['title'],
                    paper['abstract'],
                    paper['paper_type'],
                ...
                # Determine screening stage and kept_paper_id
                screening_stage = 'stage0_pass' if should_include else 'stage0_fail'
                kept_paper_id = None

                # If duplicate, find which paper to keep
                if not should_include and exclusion_reason == 'duplicate':
                    # Find the primary paper this is a duplicate of
                    for primary_id, dup_ids in duplicates.items():
                        if paper_id in dup_ids:
                            kept_paper_id = primary_id
                            break

                # Categorize rejection
                if exclusion_reason:
                    if 'duplicate' in exclusion_reason:
                        self.stats['rejected_duplicate'] += 1
                    elif 'paper_type' in exclusion_reason:
                        self.stats['rejected_wrong_type'] += 1
                    elif 'review' in exclusion_reason:
                        self.stats['rejected_review'] += 1
                    elif 'conceptual' in exclusion_reason:
