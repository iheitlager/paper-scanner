...
- Verbose pipeline output now matches validate-style format with step index, type, and params
- Replaced rich Panel header with compact one-liner
- Per-step timing always shown inline (e.g. `✓ 5/5 (1.2s)`)
- Compact final summary with total time and paper counts
...
- LLM response caching for `metadata_extraction`, `relevance_scoring`, and `camo_extraction` steps
- Two new config options per LLM step: `cache` (default: true) and `use_cache` (default: true)
...
- Make cache directories XDG Base Directory compliant — consolidate `~/.paper-scanner`, `~/.cache_files`, `~/.cache_pdf` under `$XDG_CACHE_HOME/paper-scanner/`
- Add centralized `paths.py` module for XDG-aware path resolution with legacy fallback and warnings
- Remove redundant `expanduser()` calls and fix minor inconsistencies in path resolution

## [3.8.1] - 2026-03-27

### Fixed

- Fix `load_type_mapping_config()` caching bug — default dict returned without being stored in cache, causing identity check failures
- Auto-fix 2790 ruff lint errors across 204 files (import sorting, whitespace, unused variables, formatting)
